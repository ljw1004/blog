...
NotInheritable Class App
    ...
    Sub SaveScores()
        Dim s = JsonConvert.SerializeObject(HighScores)
        ApplicationData.Current.RoamingSettings.Values("scores") = s
    End Sub

    Sub LoadScores()
        HighScores = Nothing
        Try
            If ApplicationData.Current.RoamingSettings.Values.ContainsKey("scores") Then
                Dim s = CStr(ApplicationData.Current.RoamingSettings.Values("scores"))
                HighScores = JsonConvert.DeserializeObject(Of Scores)(s)
            End If
        Catch ex As Exception
        End Try
        If HighScores Is Nothing Then HighScores = New Scores
    End Sub
